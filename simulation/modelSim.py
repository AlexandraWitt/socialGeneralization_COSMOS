                ...
                if t==0:
                    ...
                    for ag in range(nAgents):
                        ...
                        if pars[ag]['alpha']>0: #VS memory implementation: prediction error learning from posterior vs. soc info
                            ...
                            for b in vs_boost:
                                vals[b[0],ag] = vals[b[0],ag] + pars[ag]['alpha']*(b[1]*b[2]-vals[b[0],ag])
                        ...
                        #Decision biasing
                        if pars[ag]['gamma']>0:
                                #soc policy can't be 0, so we set it to a very low baseline value
...
def param_gen(nAgents,nGroups,hom=True,models=None):
    ...
    ----------
    nAgents : int
        Number of agents per group. Since we have 4 environments per set, usually use 4.
    nGroups : int
        Number of groups.
    hom : boolean, optional
        Should the groups be homogenous (all agents use the same model). The default is True.
    models : int in range(5), optional
        If specified, homogenous groups will only have the input model. 
        0=AS,1=DB,2=VS,3=SG,4=dummy (SG with eps_soc=0) model. The default is None. (Model is randomly selected)
    ...
    if hom:
        ...
        for g in range(nGroups):
            ...
            if model==1:
                par[:,3] = np.random.uniform(1/14,1,(nAgents)) #gamma
            elif model==2:
                par[:,4] = np.random.uniform(1/14,1,(nAgents)) #alpha
            elif model==3:
        ...
        for g in range(nGroups):
            ...
            for ag in range(nAgents):
                if model[ag]==1:
                    ...
                    par[ag,4] = np.random.uniform(0.5,1,(1)) #alpha
                elif model[ag]==3:
