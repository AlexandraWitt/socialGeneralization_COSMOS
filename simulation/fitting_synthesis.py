...
groupSize=len(fits)
path = "./data/fitting_data/exp_fit/"
...
#for R² we only consider choices participant choices, so random nLL depends on participant
...
medAS = np.median(np.split(AS,len(fit)),axis=1)
medDB = np.median(np.split(DB,len(fit)),axis=1)
medVS = np.median(np.split(VS,len(fit)),axis=1)
medSG = np.median(np.split(SG,len(fit)),axis=1)

medAS=pd.DataFrame(medAS,columns=["agent","group","lambda","beta","tau"])
medAS["model"]="AS"
medDB=pd.DataFrame(medDB,columns=["agent","group","lambda","beta","tau","par"])
medDB["model"]="DB"
medVS=pd.DataFrame(medVS,columns=["agent","group","lambda","beta","tau","par"]) 
medVS["model"]="VS"
medSG=pd.DataFrame(medSG,columns=["agent","group","lambda","beta","tau","par"])
medSG["model"]="SG"

pars = pd.concat((medAS,medDB,medVS,medSG))
...
for i in range(len(fit)):
    ...
    if highest==0:
        ...
        fit.loc[i,"lambda":"par"]=pars.loc[(pars["agent"]==fit.loc[i,"agent"]) & (pars["group"]==fit.loc[i,"group"]) &
                  (pars["model"]==fit.loc[i,"model"]),["lambda","beta","tau","par"]].T.to_numpy()[:,0]
fit.to_csv("./Data/model_fits.csv",index=False)

parfits.to_csv("./Data/fit+pars.csv",index=False)

# compute and save protected exceedance probability
from groupBMC.groupBMC import GroupBMC

fit = pd.read_csv("./Data/model_fits.csv")
...
result.to_csv("./Data/pxp.csv",index=False)
