            ...
            if random[t]==1:
                ...
                if pars['gamma']>0:
                    bias = [(i,np.sum(socD[np.nonzero(socD[:,1]==t-1),2]==i), 
                             np.mean(socD[np.nonzero(socD[:,1]==t-1),3][socD[np.nonzero(socD[:,1]==t-1),2]==i])) for i in socD[np.nonzero(socD[:,1]==t-1),2][0]] 
                ...
                if pars['gamma']>0:
                    ...
                    #socpolicy = exp(socChoices)
                    socpolicy = socChoices/sum(socChoices)
                    #mixture policy
                    policy = ((1 - pars['gamma'])*policy) + (pars['gamma'] * socpolicy)
                policy[policy/np.sum(policy)==0]=0.001*np.sum(policy) #if we'd get any policy values at 0, set them to a low value instead
