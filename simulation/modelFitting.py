...
subdata.loc[:,"reward"] = subdata["reward"]-0.5 #mean needs to be 0 for GP, is .5 in data
...
for ag in np.unique(subdata['agent']):
    ...
    for test in range(rounds):
        ...
        for m in range(nmodel):
            if m == 0:
                print(g,ag,m,r)
                pars = opt.differential_evolution(mf.model_fit,[(-5,3),(-5,3),(-5,3)],(m,tardata,socdata,shor),maxiter=1)['x']
                ...
                print(g,ag,m,r)
                pars = opt.differential_evolution(mf.model_fit,[(-5,3),(-5,3),(-5,3),(np.log(1/14),0)],(m,tardata,socdata,shor),maxiter=1)['x']
                fit[r,m] = mf.model_fit(pars,m,testtar,testsoc,shor)
                ...
                print(g,ag,m,r)
                pars = opt.differential_evolution(mf.model_fit,[(-5,3),(-5,3),(-5,3),(np.log(1/14),0)],(m,tardata,socdata,shor),maxiter=1)['x'] #,(np.log(1/14),0)
                fit[r,m] = mf.model_fit(pars,m,testtar,testsoc,shor)
                ...
                print(g,ag,m,r)
                pars = opt.differential_evolution(mf.model_fit,[(-5,3),(-5,3),(-5,3),(-5,4.5)],(m,tardata,socdata,shor),maxiter=1)['x']
                fit[r,m] = mf.model_fit(pars,m,testtar,testsoc,shor)
